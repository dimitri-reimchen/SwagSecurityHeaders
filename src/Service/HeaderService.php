...
namespace Swag\SecurityHeaders\Service;
...
class HeaderService
{
    ...
    {
        $enabled = $config->get("SwagSecurityHeaders.config.cspEnabled", $salesChannelId);
        if (!$enabled) {
            return;
        }

        $cspValue = $config->get("SwagSecurityHeaders.config.cspValue", $salesChannelId);
    ...
    {
        $enabled = $config->get("SwagSecurityHeaders.config.cspReportOnlyEnabled", $salesChannelId);
        if (!$enabled) {
            return;
        }

        $cspReportOnlyValue = $config->get("SwagSecurityHeaders.config.cspReportOnlyValue", $salesChannelId);
    ...
    {
        $enabled = $config->get("SwagSecurityHeaders.config.permissionsPolicyEnabled", $salesChannelId);
        if (!$enabled) {
            return;
        }

        $permissionsPolicyValue = $config->get("SwagSecurityHeaders.config.permissionsPolicyValue", $salesChannelId);
    ...
    private function addStrictTransportSecurity(Response $response, SystemConfigService $config, ?string $salesChannelId): void
    {
        ...
        $enabled = $config->get("SwagSecurityHeaders.config.hstsEnabled", $salesChannelId);
        if (!$enabled) {
            return;
        }
        ...
        if (!$response->headers->has("Strict-Transport-Security")) {
            $hstsValue = $config->get("SwagSecurityHeaders.config.hstsValue", $salesChannelId);
            if (!empty($hstsValue)) {
    ...
    {
        $enabled = $config->get("SwagSecurityHeaders.config.xFrameOptionsEnabled", $salesChannelId);
        if (!$enabled) {
            return;
        }
        ...
        if (!$response->headers->has("X-Frame-Options")) {
            $xFrameOptionsValue = $config->get("SwagSecurityHeaders.config.xFrameOptionsValue", $salesChannelId);
    ...
    {
        $enabled = $config->get("SwagSecurityHeaders.config.xContentTypeOptionsEnabled", $salesChannelId);
        if (!$enabled) {
            return;
        }
        ...
        if (!$response->headers->has("X-Content-Type-Options")) {
            $xContentTypeOptionsValue = $config->get("SwagSecurityHeaders.config.xContentTypeOptionsValue", $salesChannelId);
    ...
    {
        $enabled = $config->get("SwagSecurityHeaders.config.referrerPolicyEnabled", $salesChannelId);
        if (!$enabled) {
            return;
        }
        ...
        if (!$response->headers->has("Referrer-Policy")) {
            $referrerPolicyValue = $config->get("SwagSecurityHeaders.config.referrerPolicyValue", $salesChannelId);
